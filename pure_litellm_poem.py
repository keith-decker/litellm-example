...
def generate_poem_with_openai(api_key, proxy_url):
    """Generate a poem using OpenAI's GPT-4 through LiteLLM proxy."""
    try:
        messages = [
...
def main():
    ...
    # Get LiteLLM proxy URL from environment variable (default to localhost:4000)
    litellm_proxy_url = os.getenv("LITELLM_PROXY_URL", "http://localhost:4000")
    litellm.callbacks = ["otel"]
    
    print("=== OpenTelemetry Poetry Generator ===\n")
    print(f"Using LiteLLM Proxy: {litellm_proxy_url}\n")
    
    if openai_api_key:
        print("Generating poem with OpenAI GPT-4 (via LiteLLM proxy)...\n")
        openai_poem = generate_poem_with_openai(openai_api_key, litellm_proxy_url)
